<script>
  ...
  function checkCompletion() {
    for (const t of tiles) {
      if (tileIntersectsStopSign(t) && t.depth < maxDepth) return;
    }
    completed = true;
  }
  function initGrid() {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        tiles.push({ x: c * tileW, y: r * tileH, w: tileW, h: tileH, depth: 0 });
      }
    }
  }
  initGrid();
  ...
  function rectPolyIntersect(rx, ry, rw, rh, poly) {
    if (
      rx + rw < STOP_BOUNDS.minX ||
      rx > STOP_BOUNDS.maxX ||
      ry + rh < STOP_BOUNDS.minY ||
      ry > STOP_BOUNDS.maxY
    )
      return false;
    if (
      pointInPoly(rx, ry, poly) ||
      pointInPoly(rx + rw, ry, poly) ||
      pointInPoly(rx, ry + rh, poly) ||
      pointInPoly(rx + rw, ry + rh, poly)
    )
      return true;
    for (const p of poly) {
    ...
    for (let i = 0; i < poly.length; i++) {
      ...
      for (const e of rectEdges) {
        if (segsIntersect(a.x, a.y, b.x, b.y, e[0], e[1], e[2], e[3])) return true;
      }
    }
    return false;
  }
  function tileIntersectsStopSign(t) {
    return rectPolyIntersect(t.x, t.y, t.w, t.h, STOP_SIGN_POLY);
  }
  function onTileClick(tile) {
    if (!tileIntersectsStopSign(tile)) return;
    if (tile.depth < maxDepth) {
      clickCount++;
      const halfW = tile.w / 2,
        halfH = tile.h / 2;
      const children = [
        { x: tile.x, y: tile.y, w: halfW, h: halfH, depth: tile.depth + 1 },
        { x: tile.x + halfW, y: tile.y, w: halfW, h: halfH, depth: tile.depth + 1 },
        { x: tile.x, y: tile.y + halfH, w: halfW, h: halfH, depth: tile.depth + 1 },
        { x: tile.x + halfW, y: tile.y + halfH, w: halfW, h: halfH, depth: tile.depth + 1 },
      ];
      tiles = tiles.filter((t) => t !== tile).concat(children);
      subdivisionCount++;
      checkCompletion();
    }
  }
  ...
  function updateDimensions() {
    ...
    const newWidth = Math.min(gridWidth, avail);
    displayWidth = newWidth;
    displayHeight = Math.round(newWidth * (imgHeight / imgWidth));
  }
  onMount(() => {
    updateDimensions();
  ...
  <div
    ...
    {#if completed}
      <div
        ...
        <div class="text-[0.6rem] text-slate-500">
          Clicks: {clickCount} | Sub: {subdivisionCount}
        </div>
        <button
